...
export function GetName(name){
  return async (dispatch) =>{
      return await axios.get(`http://localhost:3001/country?nombre=` + name)
      .then(res => dispatch({
          type: BUSQUEDA,
          payload: res.data
      }))
...
return async (dispatch) => {
  return axios.delete("http://localhost:3001/actividad/delete/"+id)
 .then(res => dispatch({
  ...
  export function addActividad(Actividad){
    return async function (dispatch){
      const data = await axios.post("http://localhost:3001/actividad/",(Actividad))
      return data;
  }
